...
public class IOF300ResultListExporterTest {
  ...
  public void testDialog() throws ProcessingException {
    ...
    start.getFormatField().setValue(ImportExportFormatCodeType.IOFDataStandard300ResultsCode.ID);
  ...
  public void testExportWithEntries() throws ProcessingException {
    ...
    try {
      ...
      ResultList resultList = (ResultList) result;
      Assert.assertEquals("Version", IOF300Utility.getIOFVersion(), resultList.getIofVersion());
      Assert.assertEquals("1 Class", 1, resultList.getClassResult().size());
      ClassResult classResult = resultList.getClassResult().get(0);
      Assert.assertEquals("1 Entry", 1, classResult.getPersonResult().size());
      Assert.assertEquals("Event", "" + event.getEventNr(), resultList.getEvent().getId().getValue());
  ...
  private String doExport() throws ProcessingException {
    ...
    start.getFormatField().setValue(ImportExportFormatCodeType.IOFDataStandard300ResultsCode.ID);
    Assert.assertFalse(start.getImportExportGroup().getValue());
    // TODO MIG // TODO MIG Assert.assertFalse(start.getFileField().isTypeLoad());
    String file = IOUtility.getTempFileName(".xml");
    start.getFileField().setValue(new BinaryResource(file, null));
    Assert.assertTrue(start.getEventField().isEnabled());
